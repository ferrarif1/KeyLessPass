...
CDR v3 使用 RFC 8785 JSON 规范化，显式记录 Vault、服务、账号、凭据代际、Root Key 代际、策略/编码器版本、轮换契约、证据集、操作 ID、父哈希及副本元数据。编码器使用 HMAC 确定性流、拒绝采样和无取模偏差的 Fisher--Yates 索引选择；这不等于已证明在全部合法密码上整体均匀。

密码轮换不是 two-phase commit。协议把远端状态表示为 `仅旧凭据`、`仅新凭据`、`两者都有效`、`两者都无效` 的可能集合，每个带端点标识的认证探测只能单调收缩该集合。原子替换契约要求得到确定的`仅新凭据`证据；先并存后撤销契约允许`两者都有效`作为计划内中间态，撤销旧凭据后必须丢弃旧证据并重新探测；不透明契约不自动提交。对未知的部分认证端点做有限观察，在没有覆盖或收敛假设时无法证明全局`仅新凭据`；这是协议显式保留的不可判定边界。
