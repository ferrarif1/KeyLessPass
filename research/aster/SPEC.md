...
The semantic evaluator reuses the repository's EPSCD compiler, arbitrary-precision suffix counts, Rank/Unrank bijection, and FF1-AES-256 cycle-walking backend. The evaluator API releases only the derived credential. Its process-local Root-Epoch storage is not an MPC claim.
