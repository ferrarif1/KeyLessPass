...
static void set_window_icon(GtkWindow* window) {
  ...
  g_autofree gchar* asset_icon =
      g_build_filename(executable_dir, "data", "flutter_assets", "assets",
                       "logo.png", nullptr);
  g_autofree gchar* dev_icon =
      g_build_filename(executable_dir, "..", "..", "..", "runner", "resources",
                       "app_icon.png", nullptr);

  const gchar* candidates[] = {bundled_icon, asset_icon, dev_icon};
  for (const gchar* candidate : candidates) {
    if (!g_file_test(candidate, G_FILE_TEST_IS_REGULAR)) {
      continue;
    }
    g_autoptr(GError) error = nullptr;
    if (gtk_window_set_icon_from_file(window, candidate, &error)) {
      return;
    }
  }
}
...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "KeyLessPass");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "KeyLessPass");
  }
