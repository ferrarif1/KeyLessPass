use crate::domain::{EncodingDescriptor, FixedPosition, RequiredClass};
...
/// Canonical policy subset for EPSCD scheme version 1.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
...
impl PolicySpec {
    pub fn from_encoding_descriptor(descriptor: &EncodingDescriptor) -> Result<Self> {
        if descriptor.normalization != "none" {
            return Err(KeylessPassError::Validation(
                "EPSCD scheme version 1 supports normalization=none only".to_string(),
            ));
        }
        let fixed_characters = descriptor
            .fixed_positions
            .iter()
            .map(fixed_character_from_v2)
...
fn class_from_v2(class: &RequiredClass) -> CharacterClassConstraint {
...
fn fixed_character_from_v2(fixed: &FixedPosition) -> Result<FixedCharacterConstraint> {
    let mut characters = fixed.character.chars();
    let character = characters.next().ok_or_else(|| {
        KeylessPassError::Validation("fixed character must not be empty".to_string())
    })?;
    if characters.next().is_some() {
        return Err(KeylessPassError::Validation(
            "fixed character must contain one Unicode scalar value".to_string(),
        ));
    }
...
/// EPSCD's completion-count tables. This is exposed for matched algorithmic
