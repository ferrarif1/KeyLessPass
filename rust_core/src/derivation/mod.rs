...
pub fn derive_password_v3(
    ...
    let spec = PolicySpec::from_encoding_descriptor(&record.encoding_descriptor)?;
...
mod tests {
    ...
    fn complete_derivation_is_policy_compliant_and_non_repeating() {
        ...
        let policy = CompiledPolicy::compile(
            PolicySpec::from_encoding_descriptor(&record.encoding_descriptor).unwrap(),
        )
        .unwrap();
    ...
    fn generation_is_not_part_of_the_tweak() {
        let mut left = record();
        let policy_hash = PolicySpec::from_encoding_descriptor(&left.encoding_descriptor)
            .unwrap()
            .policy_hash()
            .unwrap();
